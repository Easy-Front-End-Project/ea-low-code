...
/**
 * 获取所有图片分组列表
 * @returns {Promise<Array<{id: number, name: string, description: string, imageCount: number, isActive: boolean}>>} 分组列表
 */
 ...
 * @param {string} data.name - 分组名称（2-50字符，必填）
 * @param {string} [data.description] - 分组描述（可选，最多200字符）
 * @returns {Promise<Object>} 创建成功后的分组信息
 */
 ...
 * @param {number} [params.pageSize=20] - 每页条数（最大50）
 * @param {number|null} [params.groupId=null] - 分组 ID（null表示全部）
 * @param {string} [params.keyword=''] - 搜索关键词（匹配文件名和描述）
 * @returns {Promise<{list: Array, total: number, page: number, pageSize: number}>} 图片列表和分页信息
 */
...
/**
 * 获取图片详细信息
 * @param {number|string} id - 图片 ID
 * @returns {Promise<Object>} 图片详细信息（含关联的分组信息）
 */
 ...
 * @param {FormData} formData - 表单数据，包含以下字段：
 ...
 * @returns {Promise<Object>} 上传成功后的图片信息（含 URL、文件名等）
 */
 ...
 * @param {number} data.id - 图片 ID（必填）
 * @param {string} [data.filename] - 新文件名（自动保留扩展名）
 * @param {number|null} [data.groupId] - 新分组 ID（null表示移出分组）
 * @param {string} [data.alt] - 新描述文字
