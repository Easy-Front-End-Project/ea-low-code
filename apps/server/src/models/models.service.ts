...
export class ModelsService {
  ...
  async findAll(keyword?: string, userId?: number): Promise<DynamicModel[]> {
    ...
    if (keyword) {
      queryBuilder.andWhere('(model.name LIKE :keyword OR model.description LIKE :keyword)', {
        keyword: `%${keyword}%`,
      })
    }
  ...
  async findModelWithFields(id: number): Promise<{ model: DynamicModel; fields: ModelField[] }> {
  ...
  async createModel(dto: CreateModelDto, userId?: number): Promise<DynamicModel> {
  ...
  async updateModel(id: number, dto: Partial<CreateModelDto>): Promise<DynamicModel> {
    const model = await this.findOne(id)
  ...
  async createField(dto: CreateFieldDto): Promise<ModelField> {
    ...
    if (existingField) {
      throw new BadRequestException(`字段名 "${dto.fieldName}" 在此模型中已存在`)
    }
    ...
    try {
      ...
      await queryRunner.addColumn(model.tableName, new TableColumn(columnOptions))
  ...
  async updateField(id: number, dto: Partial<CreateFieldDto>): Promise<ModelField> {
    ...
    if (Object.keys(updateData).length > 0) {
      ...
      if (hasSchemaChange) {
        ...
        try {
          const ormType = FIELD_TYPE_MAP[dto.fieldType || field.fieldType] || 'varchar'
          const length = dto.fieldLength ?? field.fieldLength ?? DEFAULT_FIELD_LENGTHS[ormType]
          ...
          await queryRunner.commitTransaction()
          this.logger.log(`修改字段 ${field.fieldName} 在表 ${model.tableName}`)
        } catch (error) {
          await queryRunner.rollbackTransaction()
          throw error
        } finally {
  ...
  async sortFields(modelId: number, fieldIds: number[]): Promise<void> {
    for (let i = 0; i < fieldIds.length; i++) {
      await this.fieldRepo.update({ id: fieldIds[i], modelId }, { sortOrder: i + 1 })
    }
  }

  buildEntitySchema(model: DynamicModel, fields: ModelField[]): EntitySchema<any> {
