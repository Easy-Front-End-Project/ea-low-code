      ...
      <div v-else class="p-4 space-y-6">
        ...
        <div class="component-info pb-4 border-b border-gray-100">
          <div class="flex items-center justify-between mb-2">
            ...
            <ea-button type="danger" size="small" icon="icon-trash-empty" @click="handleDeleteComponent">
              删除
            </ea-button>
          </div>
        ...
        <!-- 目标插槽配置（排在最上面） -->
        <SlotConfig :parent-slots="parentComponentMeta?.slots" :slot-value="selectedComponent.props?.slot || 'default'"
          @slot-change="handleSlotChange" />

        <!-- 插槽 Scope 数据绑定配置 -->
        <SlotScopeConfig :parent-slots="parentComponentMeta?.slots"
          :slot-value="selectedComponent.props?.slot || 'default'" :scope="selectedComponent.props?.scope || ''"
          @scope-change="handleScopeChange" />

        <!-- 特殊配置：根据组件类型动态渲染对应的配置组件 -->
        <SpecialConfigRenderer v-if="hasSpecialConfig" :component="selectedComponent"
          :special-config="componentMeta?.specialConfig" />

        <!-- 属性配置 -->
        <PropConfig :props="componentPropsWithoutSlot" :component-props="selectedComponent.props"
          @prop-change="handlePropChange" />

        <!-- 样式配置 -->
        <StyleConfig :component-type="selectedComponent.type" :component-props="selectedComponent.props"
          :style="selectedComponent.style" :css-variables="selectedComponent.cssVariables"
          @style-change="handleStyleChange" @css-variable-change="handleCssVariableChange" />

        <!-- 事件配置 -->
        <EventConfig :events="componentMeta?.events" :component-events="selectedComponent.events"
