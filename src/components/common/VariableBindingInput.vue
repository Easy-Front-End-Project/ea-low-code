<template>
  <div class="variable-binding-input">
    ...
    <div class="input-wrapper">
      ...
      <!-- 普通输入框 -->
      <template v-else>
        <EaInput v-if="inputType === 'input'" v-model="inputValue" size="small" :placeholder="placeholder"
          @input="handleInputChange" />
        <EaSelect v-else-if="inputType === 'select'" :model-value="inputValue" size="small" :placeholder="placeholder"
          @change="handleInputChange">
          <ea-option v-for="option in options" :key="option.value" :value="option.value">{{ option.label }}</ea-option>
        </EaSelect>
        <EaSwitch v-else-if="inputType === 'switch'" :model-value="switchValue"
          @update:modelValue="handleSwitchChange" />
        <UnitInput v-else-if="inputType === 'unit'" :value="inputValue" :placeholder="placeholder"
          @update:value="handleInputChange" />
        <ea-color-picker v-else-if="inputType === 'color'" :value="inputValue" @change="handleColorChange"
          class="prop-input flex-1" />
        <!-- Array/Object 类型使用按钮打开编辑器 -->
        <ea-button v-else-if="inputType === 'array' || inputType === 'object'" class="w-full text-center flex-1"
          type="primary" size="small" icon="icon-edit" @click="handleOpenEditor">
          {{ editorButtonText }}
        </ea-button>
      </template>

      <!-- 绑定变量按钮 -->
      <ea-button type="text" size="small" class="bind-btn" :class="{ 'is-bound': isVariable }"
    ...
    <!-- 变量选择对话框 -->
    <VariableSelector :visible="selectorVisible" @select="handleSelectVariable" @close="selectorVisible = false" />

    <!-- MonacoEditor 弹窗 -->
    <ea-dialog :visible="editorVisible" :title="editorTitle" width="600px" @close="handleCloseEditor">
